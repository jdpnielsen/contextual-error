# Contextual Error: rich errors

The Contextual Error module is a rewrite of Joyent's [verror](https://github.com/joyent/node-verror) in Typescript. The port is _not_ a drop in replacement, as several features has been dropped, while others have been added.

This module extends the base Error class with several features which makes the Error class better suited for operational errors in modern Javascript workflows using promises in a async/await workflow. Please see Joyent's [Best Practices for
Error Handling in Node.js](http://www.joyent.com/developers/node/design/errors) if you find any of the behavior here confusing or surprising.
...
* **ContextualError**, for chaining errors while preserving each one's error message.
...
First, install the package:
    ...
    npm install contextual-error

If nothing else, you can use ContextualError as a drop-in replacement for the built-in
JavaScript Error class:

```javascript
var err = new ContextualError('missing file: "/etc/passwd"');
...
```javascript
var fsPromises = require('fsPromises');
var filename = '/nonexistent';

try {
  const stats = await fsPromises.stat(filename); 
} catch (err1) {
	var err2 = new ContextualError(err1, `stat "${filename}"`);
...
program's name to the ContextualError's `message`.  Or just call
[node-cmdutil.fail(your_ContextualError)](https://github.com/joyent/node-cmdutil), which
does this for you.

You can get the next-level Error using `ContextualError.cause()`:

```javascript
console.error(ContextualError.cause(err2).message);
```
...
var err2 = new ContextualError(err1, 'failed to stat "%s"', '/junk');
var err3 = new ContextualError(err2, 'request failed');
...
The two main goals for ContextualError are:
...
That's why ContextualError supports `causes`.
...
Errors with a ContextualError to add information without breaking the error handling
...
* The error's name might change.  People typically use `name` to determine what
  kind of Error they've got.  To ensure compatibility, you can create ContextualErrors
  with custom names, but this approach isn't great because it prevents you from
  representing complex failures.  For this reason, ContextualError provides
...
* The error's properties might change.  People often hang additional properties
  ...
  these should be copied.  For these reasons, ContextualError first-classes these
  information properties.  You have to provide them in the constructor, you can
  only fetch them with the `info()` function, and ContextualError takes care of making
...
When the caller uses `ContextualError.info()`, the information properties are collapsed
...
supporting the complex case above, you can still just do:
   ...
   new ContextualError("my service isn't working");

for the simple cases.


# Reference: ContextualError, WError, SError

ContextualError, WError, and SError are convenient drop-in replacements for `Error` that
...
The ContextualError constructor has several forms:
 ...
 */
new ContextualError(options, sprintf_args...)

/*
 * This is a useful shorthand when the only option you need is "cause".
 */
new ContextualError(cause, sprintf_args...)

/*
 * This is a useful shorthand when you don't need any options at all.
 */
new ContextualError(sprintf_args...)
```

All of these forms construct a new ContextualError that behaves just like the built-in
...
`info`           | object           | Specifies arbitrary informational properties that are available through the `ContextualError.info(err)` static class method.  See that method for details.
...
The `WError` constructor is used exactly the same way as the `ContextualError`
constructor.  The `SError` constructor is also used the same way as the
`ContextualError` constructor except that in all cases, the `strict` property is
overriden to `true.


## Public properties

`ContextualError`, `WError`, and `SError` all provide the same public properties as
JavaScript's built-in Error objects.

Property name | Type   | Meaning
------------- | ------ | -------
`name`        | string | Programmatically-usable name of the error.
`message`     | string | Human-readable summary of the failure.  Programmatically-accessible details are provided through `ContextualError.info(err)` class method.
...
the complete `message` property.  For `SError` and `ContextualError`, this message is
...
The following methods are defined on the `ContextualError` class and as exported
functions on the `ContextualError` module.  They're defined this way rather than using
methods on ContextualError instances so that they can be used on Errors not created with
`ContextualError`.

### `ContextualError.cause(err)`
...
by invoking `ContextualError.cause(err)` on each subsequent return value.  If `err` is
not a `ContextualError`, the cause is `null`.

### `ContextualError.info(err)`
...
is), you can use `ContextualError.hasCauseWithName(err, name)`.

If a vanilla error or a non-ContextualError error is passed in, then there is no cause
chain to traverse. In this scenario, the function will check the `name`
property of only `err`.

### `ContextualError.hasCauseWithName(err, name)`

Returns true if and only if `ContextualError.findCauseByName(err, name)` would return
a non-null value.  This essentially determines whether `err` has any cause in
its cause chain that has name `name`.

### `ContextualError.errorFromList(errors)`
...
`callback(ContextualError.errorFromList(errors))` when the operation is complete.


### `ContextualError.errorForEach(err, func)`
...
```javascript
var err1 = new ContextualError('something bad happened');
/* ... */
var err2 = new ContextualError({
...
console.log(ContextualError.info(err2));
...
This outputs:
    ...
    ConnectionError: failed to connect to "127.0.0.1:215": something bad happened
        at Object.<anonymous> (/home/dap/node-ContextualError/examples/info.js:5:12)
...
console.log(ContextualError.info(err3));
...
This outputs:
    ...
    RequestError: request failed: failed to connect to "127.0.0.1:215": something bad happened
        at Object.<anonymous> (/home/dap/node-ContextualError/examples/info.js:20:12)
        at Module._compile (module.js:456:26)
        at Object.Module._extensions..js (module.js:474:10)
        at Module.load (module.js:356:32)
        at Function.Module._load (module.js:312:12)
...
var err1 = new ContextualError('something bad happened');
/* ... */
var err2 = new ContextualError(err1, 'something really bad happened here');

console.log(ContextualError.fullStack(err2));
```

This outputs:

    ContextualError: something really bad happened here: something bad happened
        at Object.<anonymous> (/home/dap/node-ContextualError/examples/fullStack.js:5:12)
        ...
        at node.js:968:3
    caused by: ContextualError: something bad happened
        at Object.<anonymous> (/home/dap/node-ContextualError/examples/fullStack.js:3:12)
        at Module._compile (module.js:409:26)
        at Object.Module._extensions..js (module.js:416:10)
        at Module.load (module.js:343:32)
        at Function.Module._load (module.js:300:12)
...
`ContextualError.fullStack` is also safe to use on regular `Error`s, so feel free to use
it whenever you need to extract the stack trace from an `Error`, regardless if
it's a `ContextualError` or not.
...
`ContextualError` options are supported.  The `message` for a MultiError consists the
...
See the convenience function `ContextualError.errorFromList`, which is sometimes simpler
